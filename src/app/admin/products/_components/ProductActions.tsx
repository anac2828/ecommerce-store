...
import { deleteProduct, toggleProuctAvailability } from '../../_actions/products';
...
export function ActiveToggleDropdownItem({ id, isAvailableForPurchase }: { id: string, isAvailableForPurchase: boolean }) {
    ...
    return <DropdownMenuItem
        ...
        disabled={isPending} > {isAvailableForPurchase ? "Deactivate" : "Activate"}
    </DropdownMenuItem>
}

// DELETE
export function DeleteDropdownMenu({ id, disabled }: { id: string, disabled: boolean }) {
    const [isPending, startTransition] = useTransition();
    const router = useRouter();
